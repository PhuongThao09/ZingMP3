...
class MaterialProgressDrawable extends Drawable implements Animatable
{
    ...
    private void applyFinishTranslation(final float n, final Ring ring) {
        ...
        ring.setStartTrim((ring.getStartingEndTrim() - this.getMinProgressArc(ring) - ring.getStartingStartTrim()) * n + ring.getStartingStartTrim());
    ...
    private int evaluateColorChange(final float n, int n2, int intValue) {
        ...
        return n5 + (int)(((intValue & 0xFF) - n5) * n) | (n2 + (int)(((intValue >> 24 & 0xFF) - n2) * n) << 24 | n3 + (int)(((intValue >> 16 & 0xFF) - n3) * n) << 16 | (int)(((intValue >> 8 & 0xFF) - n4) * n) + n4 << 8);
    }
    ...
    private void setupAnimators() {
        ...
        final Animation mAnimation = new Animation() {
            public void applyTransformation(final float n, final Transformation transformation) {
                ...
                if (n <= 0.5f) {
                    mRing.setStartTrim(startingStartTrim + MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(n / 0.5f) * (0.8f - access$100));
                }
                if (n > 0.5f) {
                    mRing.setEndTrim((0.8f - access$100) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation((n - 0.5f) / 0.5f) + startingEndTrim);
                }
                mRing.setRotation(0.25f * n + startingRotation);
